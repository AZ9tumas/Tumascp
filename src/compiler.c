...
static bool rawMatch(TokenType type){
    if (!check(type))return false;
    return true;
}
...
#ifdef DEBUG_PRINT_CODE
    if (!parser.hadError){
        disassembleChunk(currentChunk(), "stdin code");
    }
#endif
}
...
static void ifStatement() {
    ...
    expression();
    consume(TOKEN_COLON, "Expect ':' after if statement condition.");
...
static void interstring(bool canAssign){
    printf("compiler interstring\n");
    
    ObjString* mainstring = copyString(parser.previous.start + 2,parser.previous.length - 3);
    printf("comp istr: %s\n",mainstring->chars);
    emitConstant(OBJ_VAL(mainstring));

    // f"hi {1+2}"

    // Handle the tokens inside the curly brackets
    
    if (*parser.current.start=='\0')return;
    printf("before inter: %s\n", parser.current.start);
    expression();
    printf("after inter: %s\n", parser.current.start);
    consume(TOKEN_RIGHT_BRACE, "Expected '}' while parsing fstring");
}

static void string(bool canAssign){
    ObjString* mainstring = copyString(parser.previous.start + 1,parser.previous.length - 2);
    //printf("comp str: %s\n",mainstring->chars);
    emitConstant(OBJ_VAL(mainstring));
}
...
ParseRule rules[] = {
    ...
    [TOKEN_INTERSTRING]   = {interstring,   NULL,   PREC_NONE},
